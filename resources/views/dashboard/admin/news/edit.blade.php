            ...
            <div class="grid w-full gap-5 px-0 lg:px-10 mx-auto md:grid-cols-12">
                <div class="col-span-12">
                    <h2 class="lg:mt-8 mb-1 text-2xl font-semibold text-gray-700">
                        Edit Berita PKKMB 2023
                    </h2>
                    <p class="text-xs lg:text-sm text-gray-400">
                        Edit Berita PKKMB 2023 yang dipilih
                    </p>
                </div>
            </div>
        </div>
        ...
        <nav class="hidden lg:block lg:mx-10 mt-5 lg:mt-8 text-sm" aria-label="Breadcrumb">
            <ol class="inline-flex p-0 list-none">
                <li class="flex items-center">
                    <a href="{{ route('admin.news.index') }}" class="text-xs lg:text-sm text-gray-400">Berita Kemahasiswaan</a>
                ...
                <li class="flex items-center">
                    <p class="text-xs lg:text-sm font-medium">Edit Berita Kemahasiswaan</p>
                </li>
            </ol>
        </nav>
                                ...
                                <div class="lg:px-4 lg:py-5">
                                    <div class="grid grid-cols-6 gap-6">
                                        ...
                                        <div class="col-span-6">
                                            <label for="title" class="block mb-1 lg:mb-3 font-medium text-gray-700 text-sm lg:text-base">Judul Berita</label>
                                            <input placeholder="Berita apa yang ingin diunggah?" type="text" style="text-transform: capitalize;" minlength="40" maxlength="65" name="title" id="title" autocomplete="title" class="block w-full py-2 lg:py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 text-sm lg:text-base" value="{{ $news->title ?? '' }}" required>
                                        ...
                                        <div class="col-span-6">
                                            <label for="description" class="block mb-1 lg:mb-3 font-medium text-gray-700 text-sm lg:text-base">Deskripsi Berita</label>
                                            <textarea placeholder="Jelaskan terkait Berita apa yang akan diunggah?" type="text" minlength="100" name="description" id="description" autocomplete="description" class="block w-full py-2 lg:py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 text-sm lg:text-base" rows="4" required>{{ $news->description ?? '' }}</textarea>
                                        ...
                                        <div class="col-span-6">
                                            <label for="thumbnail" class="block mb-1 lg:mb-3 font-medium text-gray-700 text-sm lg:text-base">Thumbnail Berita <span class="text-gray-400 font-normal text-sm">(Ukuran Gambar 1600x900)</span></label>
                                            
                                            <div class="grid grid-cols lg:grid-cols-3 md:grid-cols-2 gap-4">
                                                @forelse ($thumbnail_news as $thumbnail_item)
                                                    <div class="mb-2">
                                                        <img src="{{ url(Storage::url($thumbnail_item->thumbnail)) }}" alt="thumbnail" class="inline object-contain h-20 rounded" for="choose">

                                                        <input placeholder="Thumbnail" type="file" name="{{ 'thumbnails['.$thumbnail_item->id.']' }}" id="thumbnails" autocomplete="thumbnails" accept=".jpg, .jpeg, .png" class="block w-full py-2 lg:py-3 pl-5 mt-3 border-gray-300 rounded-md shadow-sm focus:ring-green-500 text-sm lg:text-base">
                                                    </div>
                                                @empty
                                                    {{-- empty --}}
                                                @endforelse
                                        ...
                                        <div class="col-span-6">
                                            <label for="taglines" class="block mb-1 lg:mb-3 font-medium text-gray-700 text-sm lg:text-base">Tagline</label>
                                            <div id="newTaglineRow"></div>

                                            @forelse ($tagline as $tagline_item)
                                                <input placeholder="Tagline" type="text" name="{{ ('taglines['.$tagline_item->id.']') }}" id="taglines" autocomplete="taglines" class="block w-full py-2 lg:py-3 mt-1 border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 text-sm lg:text-base" value="{{ $tagline_item->tagline ?? '' }}" required>
                                            @empty
                                                {{-- empty --}}
                                            @endforelse
...
@push('after-script')
    ...
    <script type="text/javascript">
        ...
        $("#addThumbnailRow").click(function() {
            var html = '';
            html += '<input placeholder="Thumbnail" type="file" name="thumbnail[]" id="thumbnail" autocomplete="thumbnail" accept=".jpg, .jpeg, .png" class="block w-full py-3 pl-5 mt-1 border border-gray-300 rounded-md shadow-sm focus:ring-green-500 focus:border-green-500 sm:text-sm" required>';
