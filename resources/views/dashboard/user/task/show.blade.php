...
<div class="container py-5">
    ...
    @else
        <div class="border px-4 py-4 mb-3 rounded-lg shadow-lg">
            <h5 class="mb-3 font-bold">Form Pengumpulan Tugas :</h5>
            @if ($tambahtugas->input_type == 'Text')
            <form method="POST" action="{{ route('dashboard-user.sendTask', ['tambahtugas' => $tambahtugas->id]) }}">
                @csrf
                <input type="text" name="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
