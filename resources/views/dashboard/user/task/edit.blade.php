...
<div class="container py-5">
    ...
    <div class="border px-4 py-4 mb-3 rounded-lg shadow-lg">
        ...
        <form method="POST" action="{{ route('dashboard-user.updateTask', ['tambahtugas' => $tambahtugas->id]) }}">
            @csrf
            <input type="link" name="text" value="{{ $task->text }}" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
        ...
        @else
        <form method="POST" action="{{ route('dashboard-user.updateTask', ['task' => $task->id]) }}">
            @csrf
            <input type="file" name="file" value="{{ $task->text }}"  accept=".jpg, .jpeg, .png, .pdf" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full">
            <button type="submit" class="text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 mt-2 flex items-end mx-auto justify-end">Simpan</button>
        </form>    
        @endif
    </div>
