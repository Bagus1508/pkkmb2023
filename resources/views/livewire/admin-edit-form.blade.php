            ...
            <div class="w-100">
                <div class="mb-3">
                    <x-form-label id="name{{ $student['id'] }}"
                        label="Nama {{ $loop->iteration }} (ID: {{ $student['id'] }})" class="block mb-2 text-sm font-medium text-gray-900"/>
                ...
                <div class="mb-3">
                    <x-form-label id="nim{{ $student['id'] }}" label='NIM {{ $loop->iteration }}' class="block mb-2 text-sm font-medium text-gray-900"/>
                    <x-form-input id="nim{{ $student['id'] }}" name="nim{{ $student['id'] }}" type="nim"
                        wire:model.defer="students.{{ $loop->index }}.nim" placeholder="NIM" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"/>
